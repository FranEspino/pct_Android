...
class SharedPreferenceContext(val context: Context) {
    ...
    fun setChangePass(pass:Int){
        storage.edit().putInt(CHANGEPASS, pass).apply()
    }
    fun getChangePass():Int{
        return  storage.getInt(CHANGEPASS, 0)
    }


    //CURRENT PASS
    val CURRENTPASS = "currentpass"
    fun setCurrentPass(pass: String){
        storage.edit().putString(CURRENTPASS, pass).apply()
    }
    fun getCurrentPass():String{
        return  storage.getString(CURRENTPASS,"")!!
    }
}